...
export default function MarketSentiment({ stats }: Props) {
  ...
  const color = avg > 10 ? 'text-emerald-500' : avg < -10 ? 'text-red-500' : 'text-muted'
  ...
  return (
    <div className="rounded-xl border panel p-4">
      ...
      <div className="flex h-2 rounded-full overflow-hidden bg-zinc-100 dark:bg-[#1a2340] mb-2">
        <div className="bg-emerald-500 transition-all" style={{ width: `${(bullish / total) * 100}%` }} />
        <div className="bg-zinc-300 dark:bg-zinc-600 transition-all" style={{ width: `${(neutral / total) * 100}%` }} />
      ...
      <div className="flex justify-between text-[10px]">
        ...
        <span className="text-red-500">看空 {bearish}</span>
      </div>
    </div>
  )
}
