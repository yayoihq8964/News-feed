...
export default function TopStocks({ analyses }: Props) {
  const top = useMemo(() => {
    ...
    for (const a of analyses) for (const s of a.affected_stocks ?? []) {
      if (!map[s.ticker]) map[s.ticker] = { total: 0, count: 0, company: s.company }
      map[s.ticker].total += s.impact_score; map[s.ticker].count += 1
    }
    return Object.entries(map).map(([ticker, v]) => ({ ticker, avg: Math.round(v.total / v.count), count: v.count, company: v.company }))
            ...
            <div className="flex-1 flex items-center h-4">
              {s.avg >= 0 ? (
                <><div className="w-1/2" /><div className="w-1/2 flex items-center">
                  <div className="h-1.5 rounded-r-full bg-emerald-500 transition-all duration-500" style={{ width: `${(s.avg / maxAbs) * 100}%` }} />
                </div></>
              ) : (
                <><div className="w-1/2 flex items-center justify-end">
                  <div className="h-1.5 rounded-l-full bg-rose-500 transition-all duration-500" style={{ width: `${(Math.abs(s.avg) / maxAbs) * 100}%` }} />
                </div><div className="w-1/2" /></>
              )}
            </div>
            <span className={`font-mono text-[10px] font-semibold w-8 text-right ${s.avg >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
              {s.avg > 0 ? '+' : ''}{s.avg}
            </span>
          </div>
        ))}
