...
export default function NewsTimelineItem({ item }: Props) {
  ...
  return (
    <div className={`relative flex gap-4 py-3 border-b timeline-border cursor-pointer group ${
      item.is_pinned
        ? 'bg-amber-50/70 dark:bg-amber-500/[0.05] border-l-[3px] border-l-amber-400 pl-3 shadow-sm dark:ring-1 dark:ring-amber-500/10 rounded-r-xl my-1'
        : ''
    }`}
      onClick={() => setOpen(o => !o)}>
      {/* Time */}
      <div className="flex-shrink-0 w-16 pt-0.5 text-right">
        {item.is_pinned && <span className="text-[9px] text-amber-500 font-semibold block mb-0.5 animate-pin-pulse">📌 置顶</span>}
      ...
      <div className="flex-shrink-0 flex flex-col items-center pt-1.5">
        <div className={`w-2 h-2 rounded-full ${item.is_pinned ? 'bg-amber-400 ring-2 ring-amber-400/30' :
          a ? (a.classification === 'bullish' ? 'bg-teal-500' : a.classification === 'bearish' ? 'bg-rose-500' : 'bg-slate-400')
          : 'bg-slate-300 dark:bg-slate-600'
        }`} />
        <div className="w-px flex-1 bg-slate-100 dark:bg-[#1e2942]/60 mt-1" />
      </div>
      ...
      <div className="flex-1 min-w-0 pb-1">
        ...
        <h3 className="text-sm font-medium leading-snug group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors">
        ...
        {a && !open && stocks.length > 0 && (
          <div className="flex items-center gap-1.5 mt-2 flex-wrap">
            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
              a.classification === 'bullish' ? 'bg-teal-500/10 text-teal-600 dark:text-teal-400'
              : a.classification === 'bearish' ? 'bg-rose-500/10 text-rose-600 dark:text-rose-400'
              : 'pill-bg text-muted'
            }`}>
            ...
            {[...stocks].sort((x, y) => Math.abs(y.impact_score) - Math.abs(x.impact_score)).slice(0, 4).map(s => (
              <span key={s.ticker} className={`font-mono text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                s.impact_score > 0 ? 'bg-teal-500/10 text-teal-600 dark:text-teal-400'
        ...
        {!a && (
          <span className={`inline-block mt-1.5 text-[10px] italic ${
            ...
            item.analysis_status === 'failed' ? 'text-rose-400' :
            'text-muted-more'
          }`}>
        ...
        {open && a && (
          <div className="mt-3 p-4 rounded-2xl panel-subtle border space-y-3 shadow-sm">
            <div className="flex items-center gap-3">
              <div className={`text-2xl font-mono font-bold ${a.overall_sentiment >= 0 ? 'text-teal-500' : 'text-rose-500'}`}>
                {a.overall_sentiment > 0 ? '+' : ''}{a.overall_sentiment}
              </div>
              <div>
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                  a.classification === 'bullish' ? 'bg-teal-500/15 text-teal-600 dark:text-teal-400'
                  : a.classification === 'bearish' ? 'bg-rose-500/15 text-rose-600 dark:text-rose-400'
                  : 'pill-bg text-muted'
                }`}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[...stocks].sort((x, y) => Math.abs(y.impact_score) - Math.abs(x.impact_score)).map(s => (
                    <div key={s.ticker} className="flex items-center gap-1 panel border rounded-xl px-2 py-1">
                      ...
                      <span className={`font-mono text-[10px] font-semibold ${s.impact_score > 0 ? 'text-teal-500' : 'text-rose-500'}`}>
            ...
            {commodities.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {commodities.map(c => (
                    <div key={c.name} className="flex items-center gap-1 bg-yellow-50 dark:bg-yellow-500/5 border border-yellow-200 dark:border-yellow-500/20 rounded-xl px-2 py-1">
                      <span className="text-xs">{c.name}</span>
                      <span className={`font-mono text-[10px] font-semibold ${c.impact_score > 0 ? 'text-teal-500' : 'text-rose-500'}`}>
