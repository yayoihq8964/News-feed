...
export default function XSentimentPanel({ data, loading, onRefresh }: Props) {
  ...
  const fgColor = fg >= 75 ? 'text-leaf-600 dark:text-leaf-400' : fg >= 55 ? 'text-leaf-400' : fg >= 45 ? 'text-muted' : fg >= 25 ? 'text-amber-600 dark:text-amber-400' : 'text-coral-500 dark:text-coral-400'
    ...
    <div className="rounded-[2rem] panel p-6 bio-card">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onRefresh} disabled={loading}
          className="text-[10px] text-leaf-600 dark:text-leaf-400 hover:text-leaf-700 dark:hover:text-leaf-300 disabled:text-earth-400 transition-colors duration-200">
          {loading ? '加载中...' : '刷新'}
        </button>
      </div>
      ...
      {!data ? (
        ...
        <div className="space-y-3">
          ...
          <div className="h-2 rounded-full bg-gradient-to-r from-coral-500 via-amber-400 to-leaf-500 relative">
            <div className="absolute top-1/2 w-3.5 h-3.5 rounded-full bg-paper-50 dark:bg-moss-800 border-2 border-white dark:border-moss-400 shadow-md"
              style={{ left: `${fg}%`, transform: 'translate(-50%, -50%)' }} />
          </div>

          <div className="flex items-center justify-between">
            <span className="text-xs text-muted">散户情绪</span>
            <span className={`font-mono text-sm font-bold ${(data.retail_sentiment_score ?? 0) >= 0 ? 'text-leaf-600 dark:text-leaf-400' : 'text-coral-500 dark:text-coral-400'}`}>
              ...
              {tickers.slice(0, 4).map(t => (
                <div key={t.ticker} className="flex items-center justify-between py-1 border-b timeline-border last:border-0">
                  ...
                  <span className={`text-[10px] font-semibold ${
                    t.mention_sentiment === 'bullish' ? 'text-leaf-600 dark:text-leaf-400' :
